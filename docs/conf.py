...
version = '.'.join(dl.__version__.split('.', 2)[:2])
...
def linkcode_resolve(domain, info):
    def find_source():
        ...
        fn = os.path.relpath(fn, start=os.path.dirname(dl.__file__))
    ...
    if domain != 'py' or not info['module']:
        return None
    try:
        filename = 'dl/%s#L%d-L%d' % find_source()
    except Exception:
        filename = info['module'].replace('.', '/') + '.py'
    tag = 'master' if 'dev' in release else ('v' + release)
    return "https://github.com/dl/dl/blob/%s/%s" % (tag, filename)
...
latex_documents = [
    (master_doc, 'dl.tex', u'dl Documentation',
...
man_pages = [
    (master_doc, 'dl', u'dl Documentation',
     [author], 1)
]
